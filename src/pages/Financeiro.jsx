...
export default function Financeiro() {
  ...
  const loadFinancialData = async () => {
    try {
      ...
      // Carregar vendas (receitas automáticas)
      ...
      const vendas = vendasSnap.docs.map(doc => ({
        id: doc.id,
        type: 'receita',
        description: `Venda #${doc.id.substring(0, 8)} - ${doc.data().cliente || 'Cliente não informado'}`,
      ...
      const prevVendasQuery = query(
        collection(db, 'vendas'),
        where('status', '==', 'concluida'),
        where('createdAt', '>=', Timestamp.fromDate(prevMonth)),
        where('createdAt', '<=', Timestamp.fromDate(endPrevMonth))
      );
      const prevVendasSnap = await getDocs(prevVendasQuery);
      const prevReceita = prevVendasSnap.docs.reduce((acc, doc) => acc + (doc.data().total || 0), 0);
